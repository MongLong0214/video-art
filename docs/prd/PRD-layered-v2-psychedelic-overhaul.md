...
- [ ] AC-6.4: 기존 period=4, period=20은 duration=10의 약수가 아님 → **VALID_PERIODS 동적 계산**으로 duration 기반 약수만 허용. 기존 scene.json에 period=4/20이 있으면 `min(period, duration)` 중 가장 가까운 약수로 자동 조정
