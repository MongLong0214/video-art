...
- [x] G1: 레퍼런스 영상 대비 생성 영상의 **색감 충실도와 시각 품질**을 자동으로 측정하는 평가 harness 구축
- [x] G2: 28개 튜닝 파라미터를 단일 config 파일로 **외부화**
- [x] G3: `수정 → 실행 → 평가 → keep/discard` **자율 실험 루프** 구현
- [x] G4: 모든 실험의 메트릭과 config snapshot을 **results.tsv**에 기록
- [x] G5: AI 에이전트가 참조할 **program.md** (연구 지시서) 작성
- [x] G6: 최적 config를 baseline으로 승격하는 **promote 메커니즘** 구현
- [x] G7: 실험 루프를 기존 `pipeline:layers` 명령과 **호환**되게 통합
...
**Acceptance Criteria:**
- [x] AC-1.1: `prepare.ts`가 source.mp4에서 1fps 비례 샘플링으로 keyframe을 추출한다 (10초 영상 → 10장, 20초 → 20장)
- [x] AC-1.2: `evaluate.ts`가 생성 영상에서도 동일 비례 위치로 keyframe을 추출한다 (fps/길이/비율 무관)
- [x] AC-1.3: 추가로 3개의 temporal pair (연속 프레임 쌍)를 추출하여 시간축 메트릭에 사용한다
- [x] AC-1.4: 프레임 비교 시 해상도 차이가 있으면 작은 쪽 기준으로 리사이즈한 후 비교한다 (포맷 패널티 없음)
- [x] AC-1.5: 색감 메트릭 3종(Color Palette Sinkhorn, Dominant Color CIEDE2000, Color Temperature Ohno+Duv)을 계산한다
- [x] AC-1.6: 시각 품질 메트릭 3종(MS-SSIM YCbCr, Canny Edge Preservation, Bidirectional Texture Richness)을 계산한다
- [x] AC-1.7: 시간축 메트릭 2종(VMAF via ffmpeg libvmaf, Temporal Coherence)을 계산한다
- [x] AC-1.8: 레이어 분해 품질 메트릭 2종(Layer Independence, Role Coherence)을 계산한다
- [x] AC-1.9: 모든 메트릭은 `clamp01()` (0-1 범위 보장)을 적용한다
- [x] AC-1.10: 판정은 **Hard Gate + Secondary Ranking**: 모든 메트릭이 gate threshold 이상이어야 하고, 통과 시 composite score로 순위화한다
- [x] AC-1.11: 평가 harness(evaluate.ts, prepare.ts, metrics/*)는 수정 불가(read-only)로 관리된다
- [x] AC-1.12: Replicate model version을 결과에 기록하고, baseline과 version이 다르면 baseline reset을 강제한다
...
**Acceptance Criteria:**
- [x] AC-2.1: `scripts/research/research-config.ts`에 모든 tunable 상수가 정의된다
- [x] AC-2.2: 기존 모듈(candidate-extraction, layer-resolve, complexity-scoring, scene-generator 등)이 config를 import해서 사용한다
- [x] AC-2.3: config에 없는 값은 기존 하드코딩 값을 default로 유지한다 (behavioral parity)
- [x] AC-2.4: config 파일 형식은 TypeScript object export + Zod validated
- [x] AC-2.5: 기존 테스트가 모두 통과한다
- [x] AC-2.6: config 변경이 필요 없는 일반 파이프라인 실행에는 영향이 없다
...
**Acceptance Criteria:**
- [x] AC-3.1: `tsx scripts/research/run-once.ts`로 단일 실험(config 적용 → 파이프라인 → 평가 → keep/discard)을 실행한다
- [x] AC-3.2: AI 에이전트(Claude Code)가 program.md를 읽고, research-config.ts를 수정하고, run-once.ts를 반복 호출하는 것이 "루프"다 (원본 autoresearch 패턴)
- [x] AC-3.3: 루프 시작 시 `autoresearch/{tag}` 브랜치를 생성하고 그 위에서 작업한다
- [x] AC-3.4: 판정은 Hard Gate 통과 + quality_score가 baseline + δ_min 이상이면 keep, git commit한다
- [x] AC-3.5: Hard Gate 미통과 또는 quality_score가 baseline + δ_min 미만이면 discard, `git checkout -- scripts/research/research-config.ts`로 복원한다
- [x] AC-3.5a: δ_min은 calibration phase에서 측정한 2σ 이상으로 설정한다
- [x] AC-3.6: 각 실험 결과를 `results.tsv`에 기록한다
- [x] AC-3.7: 크래시 발생 시 crash로 기록하고 revert 후 다음 실험으로 진행한다
- [x] AC-3.8: 5회 연속 crash 시 자동 중단 + 진단 로그 출력
- [x] AC-3.9: 실험 루프는 수동 중단(Ctrl+C/SIGINT)까지 무한 반복한다
- [x] AC-3.10: SIGINT 수신 시 현재 실험을 graceful하게 종료하고 상태를 정리한다
...
**Acceptance Criteria:**
- [x] AC-4.1: `results.tsv`에 commit, quality_score, gate_pass, 10개 개별 메트릭(M1-M10), model_version, status, elapsed_ms, description이 기록된다
- [x] AC-4.2: 각 keep된 실험의 config snapshot이 git history에 남는다
- [x] AC-4.3: `npm run research:report`로 실험 이력 요약(best/worst/trend/avg)을 볼 수 있다
- [x] AC-4.4: report에 top-5 실험의 config diff가 포함된다
...
**Acceptance Criteria:**
- [x] AC-5.1: `scripts/research/program.md`에 setup, experimentation, output format, logging, loop 절차가 문서화된다
- [x] AC-5.2: 각 파라미터의 유효 범위, 의미, 상호의존성이 명시된다
- [x] AC-5.3: 금지사항(evaluate.ts/prepare.ts 수정, 외부 패키지 설치, 평가 harness 조작)이 명시된다
- [x] AC-5.4: 실험 전략 가이드(단일 파라미터 sweep → 조합 탐색 → 극단값 테스트)가 포함된다
- [x] AC-5.5: simplicity criterion — 동일 품질이면 더 적은 파라미터 변경이 선호됨
...
**Acceptance Criteria:**
- [x] AC-6.1: `npm run research:promote`로 현재 best config를 baseline으로 지정한다
- [x] AC-6.2: baseline 변경 시 이전 baseline은 history에 보존된다
- [x] AC-6.3: 새 실험 세트 시작 시 현재 baseline이 초기 config로 사용된다
- [x] AC-6.4: promote 시 baseline config + 해당 quality_score가 기록된다
...
- [x] frame-extractor.ts (1fps 비례 추출 + temporal pairs)
- [x] metrics/: 10개 메트릭 모듈 (M1-M10, Sinkhorn/Ohno/MS-SSIM/Canny/VMAF)
- [x] evaluate.ts (Hard Gate + Secondary Ranking + clamp01)
- [x] prepare.ts (레퍼런스 준비)
- [x] calibrate.ts (noise floor 측정 → δ_min)

### 9.2 Phase 2: Config Externalization

- [x] research-config.ts (Zod schema + defaults + multiplier 패턴 + constraints)
- [x] 기존 모듈 optional config 인자 전환 (7개 모듈, default fallback으로 기존 테스트 무수정)
- [x] 기존 테스트 전체 pass 확인

### 9.3 Phase 3: Experiment Engine

- [x] run-once.ts (단일 실험 실행기: config → pipeline → evaluate → keep/discard)
- [x] results.tsv 기록 (10 메트릭 + gate + model_version)
- [x] git branch/commit/checkout 자동화 (target file restore 패턴)
- [x] graceful shutdown (SIGINT)
- [x] crash recovery + 5회 연속 중단 + non-zero exit

### 9.4 Phase 4: Polish & Instrumentation

- [x] program.md (에이전트 연구 지시서 — 파라미터 범위, 전략, 금지사항)
- [x] report.ts (실험 이력 분석 + correlation audit + top-5 config diff)
- [x] promote.ts (baseline 승격 + model version 기록)
- [x] package.json scripts 등록
- [x] .gitignore 갱신 (.cache/research/, results.tsv)
