...
If this file conflicts with chat memory or old notes: **follow this file + `OUTPUT_GAP_ANALYSIS.md`.**
...
docs/REPRO_LOCKS_PLAYBOOK.md   # THIS file (full procedure)
OUTPUT_GAP_ANALYSIS.md         # aesthetic / gate / QA OS (still mandatory)
...
```text
[ ] A. Confirm closed slug name (rNNN-…) with case ledger entry in OUTPUT_GAP_ANALYSIS.md
...
[ ] I. Update OUTPUT_GAP_ANALYSIS.md approved finals table → git paths (not only out/layered)
[ ] J. Append/update CASE in §9 with lock paths + learning
[ ] K. git add only: sources/approved/<png> recipes/locks/* OUTPUT_GAP_ANALYSIS.md (+ AGENTS if needed)
...
1. Finish pipeline under OS (`OUTPUT_GAP_ANALYSIS.md`) until closed.
...
2. `OUTPUT_GAP_ANALYSIS.md` §1 approved tables + §9 CASE
...
# REF1/REF2: see OUTPUT_GAP_ANALYSIS.md §2
```
...
| Doc | Use for |
|-----|---------|
| `OUTPUT_GAP_ANALYSIS.md` | Aesthetics, QA thresholds, gate refs, case ledger, type→recipe |
...
**Priority:** `OUTPUT_GAP_ANALYSIS.md` (how to make loops) + **this playbook** (how to store/rebuild closed ones).
