> **SUPERSEDED archive copy.** Live agents: `docs/video-os/00-INDEX.md` → `01-CREATE-OS.md`. Do not follow peacock/tone defaults here.

# Layered Psychedelic Pipeline — Playbook & Knowledge

> ⭐ **작업 정본은 [`IMAGE_TO_LOOP_WORKFLOW.md`]** (의사결정 트리 · 소스타입별 레시피 · 실패 카탈로그 · 행동 규칙). 이 PLAYBOOK은 파이프라인 내부/톤프리셋/로컬폴백 **기술 심화 레퍼런스**로 참조.
> ⚠️ **방향 정정(2026-07)**: 아래 §4의 "colorCycle이 유저 허용 유일 모션 / 기하모션 저급" 중 *색흐름 속도*는 이제 적극적으로 올린다(빠른 색흐름=주력). 기하 모션 금지는 유효. 최신 방향은 WORKFLOW 문서 우선.
> 이미지 1장 → (선택)AI 레이어 분리 → GLSL 사이키델릭 셰이딩 → 20초 seamless 루프 mp4.
